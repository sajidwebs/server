...
async function seedDatabase() {
  try {
    ...
    const doctorClasses = await DoctorClass.bulkCreate([
      { class_name: 'Regular Prescriber', short_name: 'RP', status: 'active' },
      { class_name: 'Irregular Prescriber', short_name: 'IRP', status: 'active' },
      { class_name: 'Sampark Doctor', short_name: 'SAMP', status: 'active' },
      { class_name: 'Group B', short_name: 'B', status: 'active' }
    ]);
    console.log(`✅ Created ${doctorClasses.length} doctor classes`);
    ...
    const doctorCategories = await DoctorCategory.bulkCreate([
      ...
      { category_name: 'Group B', short_name: 'B', status: 'active' }
    ]);
    console.log('✅ Created 2 doctor categories');
    ...
    const divisions = await Division.bulkCreate([
      { division_name: 'Task Force', short_name: 'TF', code: 'DIV-TF', status: 'active' },
      { division_name: 'General', short_name: 'GEN', code: 'DIV-GEN', status: 'active' },
      { division_name: 'Ortho', short_name: 'ORTH', code: 'DIV-ORTH', status: 'active' },
      { division_name: 'Cardio', short_name: 'CARD', code: 'DIV-CARD', status: 'active' }
    ]);
    ...
    const productCategories = await ProductCategory.bulkCreate([
      ...
      { category_name: 'Gel', short_name: 'Gel', category_type: 'form', status: 'active' },
      { category_name: 'Ampoule', short_name: 'Amp', category_type: 'form', status: 'active' },
      { category_name: 'Syrup', short_name: 'Syp', category_type: 'form', status: 'active' },
      { category_name: 'Injection', short_name: 'Inj', category_type: 'form', status: 'active' }
    ]);
    console.log('✅ Created 6 product categories');
    ...
    const packSizes = await PackSize.bulkCreate([
      ...
      { pack_size: "Tube", short_name: 'Tube', display_order: 7, status: 'active' },
      { pack_size: "100ml", short_name: '100ml', display_order: 8, status: 'active' }
    ]);
    console.log('✅ Created 8 pack sizes');

    console.log('🏷️ Creating brand groups...');
    const brandGroups = await BrandGroup.bulkCreate([
      { brand_group_name: 'Osteoswift', short_name: 'OSTF', division_id: divisions[0].id, status: 'active' },
      { brand_group_name: 'Osteoswift', short_name: 'OSTF-GEN', division_id: divisions[1].id, status: 'active' }
    ]);
    console.log('✅ Created 2 brand groups');
    ...
    const strengths = await Strength.bulkCreate([
      ...
      { strength_value: '1000', unit: 'mg', short_name: '1000mg', display_order: 8, status: 'active' }
    ]);
    console.log('✅ Created 8 strengths');
    ...
    const products = await Product.bulkCreate([
      // Osteoswift - Task Force Division
      { 
        unique_id: '00123',
        name: 'Osteoswift', 
        short_name: 'Ostf', 
        ...
        pts: null, ptr: null, mrp: null,
        launch_date: null,
        status: 'active'
      },
      // Osteoswift Gold - General Division
      { 
        unique_id: '00145',
      ...
      // PCM500 - General Division (from product list example)
      { 
        unique_id: '00001',
      ...
      // OrthoCare Gel - Ortho Division (from product list example)
      { 
        unique_id: '00002',
        name: 'OrthoCare Gel', 
        short_name: 'OCG', 
        ...
        pack_size_id: packSizes[6].id, 
        strength_id: null, 
        pts: 105.00, ptr: 110.00, mrp: 120.00,
        launch_date: '2024-01-01',
        status: 'active'
      },
      // CardioPlus - Cardio Division (from product list example)
      { 
        unique_id: '00003',
        name: 'CardioPlus', 
        short_name: 'CP', 
    ...
    const sampleMaster = await SampleMaster.bulkCreate([
      { product_id: products[2]?.id, pack_size_id: packSizes[0]?.id, sample_name: 'PCM Sample', sample_qty: 2, unit: 'Tab', max_per_call: 5, status: 'active', created_by: adminUser.id },
      { product_id: products[3]?.id, pack_size_id: packSizes[3]?.id, sample_name: 'Ortho Sample', sample_qty: 10, unit: 'g', max_per_call: 2, status: 'active', created_by: adminUser.id },
      { product_id: products[4]?.id, pack_size_id: packSizes[1]?.id, sample_name: 'CardioPlus Sample', sample_qty: 5, unit: 'Tab', max_per_call: 3, status: 'active', created_by: adminUser.id },
      { product_id: products[0]?.id, pack_size_id: packSizes[1]?.id, sample_name: 'Osteoswift Sample', sample_qty: 2, unit: 'Tab', max_per_call: 5, status: 'active', created_by: adminUser.id },
      { product_id: products[1]?.id, pack_size_id: packSizes[0]?.id, sample_name: 'Osteoswift Gold Sample', sample_qty: 2, unit: 'Tab', max_per_call: 5, status: 'active', created_by: adminUser.id }
